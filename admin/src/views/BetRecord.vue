...
export default {
  ...
  methods:{
    ...
    changeok(id){
      ...
      axios.get('http://localhost:3000/api/updateBetStatus',{
        params:{
          time:time,
          isok:isok
        }
    ...
    changejoin(id){
      ...
      axios.get('http://localhost:3000/api/updateBetJoinStatus',{
        params:{
          time:time,
          isjoin:isjoin
        }
  ...
  searchType(){
    axios.get('http://localhost:3000/api/searchType',{
      params:{
...
sendcoin(id){
  ...
  let data = {'id':id}
  /*接口请求*/
  axios.post('http://localhost:3000/api/sendcoin',data).then((res)=>{
    console.log(res);
    if (res.data=='outed') {
      alert("已经返过币了！！！！！")
    }
