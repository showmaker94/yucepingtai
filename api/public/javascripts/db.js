...
exports.updateContractIsOk=(contract_id,isok)=>{
  return new Promise((resolve,reject)=>{
    ...
    Search.findOneAndUpdate({_id:sid},{isok:isok},function(err,docs){
    if (err) return reject(err);
    return resolve(docs);
  })
})
...
exports.updateContractStatus=(contract_id,currentstatus)=>{
  return new Promise((resolve,reject)=>{
    var sid=mongoose.Types.ObjectId(contract_id)
    Search.findOneAndUpdate({_id:sid},{currentstatus:currentstatus},function(err,docs){
    if (err) return reject(err);
    return resolve(docs);
  })
})
...
exports.updateContractBeginDate=(contract_id,begindate)=>{
  return new Promise((resolve,reject)=>{
    var sid=mongoose.Types.ObjectId(contract_id)
    Search.findOneAndUpdate({_id:sid},{begindate:begindate},function(err,docs){
    if (err) return reject(err);
    return resolve(docs);
  })
})
...
exports.searchCommentById=(id)=>{
  var sid=mongoose.Types.ObjectId(id)
  return new Promise((resolve,reject)=>{
    MyComment.find({'contract_id':sid}).sort({'time':-1}).exec(function (err, docs) {
      if (err) return reject(err);
      return resolve(docs);
  })
})
...
/*插入评论*/
exports.insertComment=(contract_id,time,name,comment)=>{
  return new Promise((resolve,reject)=>{
...
exports.searchBetById=(contract_id,isok)=>{
  ...
  return new Promise((resolve,reject)=>{
    ...
    obj.contract_id=sid;
    Bet.find(obj).sort({'time':-1}).exec(function (err, docs) {
      if (err) return reject(err);
      return resolve(docs);
  })
})
...
exports.searchBetByIdSelf=(contract_id,isok)=>{
  return new Promise((resolve,reject)=>{
    var sid=mongoose.Types.ObjectId(contract_id)
    Bet.find({_id:sid}).exec(function (err, docs) {
      // console.log(docs);
      if (err) return reject(err);
      return resolve(docs);
  })
...
/*插入押注*/
exports.insertBet=(id,time,bet,inaddr,innum,outaddr,outnum,weight,isok)=>{
  return new Promise((resolve,reject)=>{
    var obj = new Bet({
      contract_id:id,
      time:time,
...
/*更新Bet状态*/
exports.updateBetStatus=(time,isok)=>{
  return new Promise((resolve,reject)=>{
...
exports.updateBetJoinStatus=(time,isjoin)=>{
  return new Promise((resolve,reject)=>{
    ...
    Bet.findOneAndUpdate({time:time},{isjoin:isjoin},function(err,docs){
    if (err) return reject(err);
    return resolve(docs);
  })
})
...
exports.updateBetIsout=(contract_id)=>{
  return new Promise((resolve,reject)=>{
    var sid=mongoose.Types.ObjectId(contract_id)
    // console.log(sid);
    Bet.findOneAndUpdate({'_id':sid},{isout:'1'},function(err,docs){
    if (err) return reject(err);
    // console.log(docs);
    return resolve(docs);
  })
...
exports.updateBet=(address,amount,outnum)=>{
  return new Promise((resolve,reject)=>{
    ...
    obj.isjoin=1;
    obj.outnum=outnum;
    Bet.findOneAndUpdate({inaddr:address},{$set:obj},function(err,docs){
      if (err) return reject(err);
      return resolve(docs);
    })
  })
...
exports.searchType=(type,contract_id)=>{
  return new Promise((resolve,reject)=>{
    ...
    Bet.find({'contract_id':contract_id}).or([{'bet':{$regex : re}}]).sort({'time':-1}).exec(function (err, docs) {
      return resolve(docs);
    });
  })
}
...
exports.changeTitleOrDes=(id,title,des,nobetnum,totalnobet,yesbetnum,totalyesbet,totalbetnum,totalbet)=>{
  return new Promise((resolve,reject)=>{
    ...
    ({'_id':sid},{$set:obj},function(err,docs){
    if (err) return reject(err);
    return resolve(docs);
  })
})
